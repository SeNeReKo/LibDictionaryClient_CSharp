...
namespace LibDictionaryClient
{
	...
	/// few inserts missing.
	/// </summary>
	public class QueueUpdateExistingWords : IDisposable
	{
		...
		internal QueueUpdateExistingWords(DictionaryCollection api, int maxQueueLength)
		{
		...
		public void DisposeWithoutFlush()
		{
			queue.Clear();
		}
		...
		{
			if (word.ID == null) throw new Exception("Word has no ID!");
		...
		public void Flush()
		{
			...
			DictWord[] words = api.SetWords(out durationMS_Server, queue.ToArray());
			...
			for (int i = 0; i < words.Length; i++) {
				if (words[i] == null) throw new Exception();	// TODO: this is no longer necessary. an exception is thrown if at least one operation failed.
				// TODO: to have a different error strategy would be nice.
				// TODO: the same applies to the add-word-queue
			}
			queue.Clear();
		}
